...
namespace linq
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<string> LFruits2 = fruits.Where(fruit => fruit.StartsWith("L"));
            foreach (string word in LFruits2)
            {
                Console.WriteLine($"{word}");
            }

            // List<string> fuityfruits = new List<string>() { "Lemon", "Apple", "Orange", "Lime", "Watermelon", "Loganberry" };

            // // The query variable can also be implicitly typed by using var
            // IEnumerable<string> filteringQuery =
            //     from fruit in fruityfruits
            //     where num < 3 || num > 7
            //     select num;
